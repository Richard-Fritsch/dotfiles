...
if [[ -e /usr/local/bin/apps/joplin/VERSION ]] && [[ $(< /usr/local/bin/apps/joplin/VERSION) == "${RELEASE_VERSION}" ]]; then
    ...
    [[ -e /usr/local/bin/apps/joplin/VERSION ]] && CURRENT_VERSION=$(< /usr/local/bin/apps/joplin/VERSION)
...
# Check AppImage hash
IFS=' '
GITHUB_HASH=`cat ${TEMP_DIR}/Joplin.AppImage.sha512`
read -ra FILE_HASH <<< `sha512sum ${TEMP_DIR}/Joplin.AppImage`
if [ "$GITHUB_HASH" != "$FILE_HASH" ] 
then
  echo 'The hash of the AppImage differs from the check file. Exiting...'
  echo DIGEST hash -------- $GITHUB_HASH
  echo AppImage hash ------ $FILE_HASH
  rm -rf $TEMP_DIR
  exit 1
fi

# Delete previous version (in future versions joplin.desktop shouldn't exist)
rm -f /usr/local/bin/apps/joplin/*.AppImage ~/.local/share/applications/joplin.desktop /usr/local/bin/apps/joplin/VERSION

# Creates the folder where the binary will be stored
mkdir -p /usr/local/bin/apps/joplin/

# Download the latest version
mv ${TEMP_DIR}/Joplin.AppImage /usr/local/bin/apps/joplin/Joplin.AppImage

# Gives execution privileges
chmod +x /usr/local/bin/apps/joplin/Joplin.AppImage
...
# Record version
echo $RELEASE_VERSION > /usr/local/bin/apps/joplin/VERSION
